...
public class OverviewActivity extends Activity {
	...
	/*
	 * bissl experimentiert
	 * 
	 * Animation marquee = AnimationUtils.loadAnimation(getBaseContext(), R.anim.overview_marquee);
	 * 
	 * marquee.setRepeatMode(Animation.RESTART); marquee.setRepeatCount(Animation.INFINITE);
	 * scrollText.setAnimation(marquee); scrollText.startAnimation(marquee);
	 * 
	 * Context context = scrollText.getContext(); // gets the context of the view
	 * 
	 * measures the unconstrained size of the view before it is drawn in the layout
	 * scrollText.measure(View.MeasureSpec.UNSPECIFIED, View.MeasureSpec.UNSPECIFIED);
	 * 
	 * takes the unconstrained wisth of the view float width = scrollText.getMeasuredWidth();
	 * 
	 * gets the screen width float screenWidth = ((Activity)
	 * context).getWindowManager().getDefaultDisplay().getWidth();
	 * 
	 * // perfrms the calculation float toXDelta = width - (screenWidth - margin);
	 * 
	 * sets toXDelta to 0 if the text width is smaller that the screen size if (toXDelta < 0) {
	 * toXDelta = 0; } else { toXDelta = 0 - toXDelta; }
	 * 
	 * // Animation parameters Animation mAnimation = new TranslateAnimation(0, toXDelta, 0, 0);
	 * mAnimation.setDuration(10000); mAnimation.setRepeatMode(Animation.RESTART);
	 * mAnimation.setRepeatCount(Animation.INFINITE); //
	 * scrollText.setCompoundDrawablesWithIntrinsicBounds(
	 * getResources().getDrawable(R.drawable.cat_360), null, null, null);
	 * 
	 * scrollText.startAnimation(mAnimation); scrollText.setSelected(true);
	 * 
	 * try { CwNavigationMainTabActivity.getApiCaller().authenticateOnCW(); this.getCurrentNews(); }
	 * catch (ConsolewarsAPIException e) { Log.e(getString(R.string.exc_auth_tag), e.getMessage(),
	 * e); e.printStackTrace(); } styleStringBuilder = new StyleSpannableStringBuilder();
	 * 
	 * for (News news : this.news) { // get the table row by an inflater and set the needed
	 * information final View tableRow = LayoutInflater.from(this).inflate(R.layout.news_row_layout,
	 * overview, false);
	 * 
	 * tableRow.setId(news.getId());
	 * 
	 * ((ImageView) tableRow.findViewById(R.id.news_row_category_icon))
	 * .setImageResource(createCategoryIcon(news.getCategoryshort())); ((TextView)
	 * tableRow.findViewById(R.id.news_row_title)).setText(createTitle(news .getTitle()));
	 * ((TextView) tableRow.findViewById(R.id.news_row_date)).setText(createDate(news .getUnixtime()
	 * * 1000L)); TextView author = (TextView) tableRow.findViewById(R.id.news_row_author);
	 * author.setSelected(true); author.setText(createCommentAndAuthor(news.getComments(),
	 * news.getAuthor()));
	 * 
	 * newsTable.addView(tableRow); } styleStringBuilder = null;
	 */
}
