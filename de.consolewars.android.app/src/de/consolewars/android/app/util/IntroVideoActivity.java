...
/**
 * <p>
 * Activity that will play a video from YouTube. A specific video or the latest video in a YouTube
 * playlist can be specified in the intent used to invoke this activity. The data of the intent can
 * be set to a specific video by using an Intent data URL of the form:
 * </p>
 * 
 * <pre>
 *     ytv://videoid
 ...
 * 
 * <p>
 * If the user wishes to play the latest video in a YouTube playlist, the Intent data URL should be
 * of the form:
 * </p>
 * 
 * <pre>
 ...
 * 		IntroVideoActivity.class);
 * startActivity(lVideoIntent);
 * </pre>
 * 
 * <p>
 * There are several messages that are displayed to the user during various phases of the video load
 * process. If you wish to supply text other than the default english messages (e.g.,
 * internationalization, etc.), you can pass the text to be used via the Intent's extended data. The
 * messages that can be customized include:
 * 
 * <ul>
...
public class IntroVideoActivity extends Activity {
	...
	/**
	 * Task to figure out details by calling out to YouTube GData API. We only use public methods
	...
	private class QueryYouTubeTask extends AsyncTask<YouTubeId, ProgressUpdateInfo, Uri> {
		...
		protected Uri doInBackground(YouTubeId... pParams) {
			...
			try {
				...
				WifiManager lWifiManager = (WifiManager) IntroVideoActivity.this
						.getSystemService(Context.WIFI_SERVICE);
				TelephonyManager lTelephonyManager = (TelephonyManager) IntroVideoActivity.this
						.getSystemService(Context.TELEPHONY_SERVICE);

				// //////////////////////////
				// if we have a fast connection (wifi or 3g), then we'll get a high quality YouTube
				// video
				if ((lWifiManager.isWifiEnabled() && lWifiManager.getConnectionInfo() != null && lWifiManager
						.getConnectionInfo().getIpAddress() != 0)
						|| (lTelephonyManager.getNetworkType() == TelephonyManager.NETWORK_TYPE_UMTS && lTelephonyManager
								.getDataState() == TelephonyManager.DATA_CONNECTED)) {
					lYouTubeFmtQuality = "18";
				}

				// /////////////////////////////////
				// if the intent is to show a playlist, get the latest video id from the playlist,
				// otherwise the video
				...
				Log.e(this.getClass().getSimpleName(),
						"Error occurred while retrieving information from YouTube.", e);
			}
		 ...
		 */
		private String calculateYouTubeUrl(String pYouTubeFmtQuality, String pYouTubeVideoId)
			...
			lUriStr = "http://www.youtube.com/get_video?video_id=" + pYouTubeVideoId + "&t="
					+ URLEncoder.encode(lTokenStr) + "&fmt=" + pYouTubeFmtQuality;
			return lUriStr;
		}
		 ...
		 */
		private String queryLatestPlaylistVideo(PlaylistId pPlaylistId) throws IOException,
			...
			HttpGet lGetMethod = new HttpGet(YOUTUBE_PLAYLIST_ATOM_FEED_URL + pPlaylistId.getId()
