...
public class DateUtil {
	...
	/**
	 * relative time passed (current version with german time text) Example output: vor 34 Minuten
	 * (34 minutes ago)
	 * 
	 * @author cerpin (arrewk@gmail.com)
	 * @param date
	...
	public static String timePassed(Date date) {
		...
		if (secondsPassed > MONTH)
			return getTimeText(secondsPassed / MONTH, MONTH);
		if (secondsPassed > WEEK)
			return getTimeText(secondsPassed / WEEK, WEEK);
		if (secondsPassed > DAY)
			return getTimeText(secondsPassed / DAY, DAY);
		if (secondsPassed > HOUR)
			return getTimeText(secondsPassed / HOUR, HOUR);
		if (secondsPassed <= HOUR - MINUTE)
			return getTimeText(secondsPassed / MINUTE, MINUTE);
